    ...
    return (
        <>
            <Box>
                <Box pt={2}>
                    ...
                    <Box display='flex' flexWrap='wrap' gap={2}>
                        {shipments.length === 0 && <Box display='flex' justifyContent='center' alignItems='center' width='100%' height='100%' flexDirection='column'>
                            <Typography variant="h6" color='gray'>لا توجد شحنات</Typography>
                            </Box>}
                        {shipments.map(shipment => (
                            <Card key={shipment._id} dir="rtl" sx={{ width: '100%', maxWidth: 400, padding: 2 }}>
                                <CardContent>
                                    <Box>
                                        <Box display='flex' justifyContent='space-between' alignItems='space-between'>
                                            ...
                                            <Box border={'1px solid #ccc'} borderRadius='8px' padding={1.5} display='flex' justifyContent='space-between' alignItems='center'>
                                                <Typography variant='h6'>{shipment.total - shipment.shipmentFees} شيكل</Typography>
                                            </Box>
                                        </Box>
                                        <Box display='flex' alignItems='center'>
                                            <Typography variant="body1" marginInlineEnd={2}>الحالة:</Typography>
                                            <Chip size='small' color={ShipmentStatusColors[shipment.status]} label={ShipmentStatusLabels[shipment.status]} />
                                        </Box>
                                        <Typography variant="h6" mt={2}>المجموع: {shipment.total} شيكل</Typography>
                                        <Typography variant="h6" mt={2}>التوصيل: {shipment.shipmentFees} شيكل</Typography>
                                    </Box>
                                    <Box mt={2}>
                                            ...
                                            {shipment.content.map(contentItem => (
                                                <ListItem key={contentItem._id} style={{ padding: '0', margin: '16px 0' }}>
                                                    • <ListItemText
                                                        ...
                                                        secondary={<Typography variant="body2">القياس: {contentItem.size} | اللون: {contentItem.color}</Typography>}
                                                    />
                                                </ListItem>
                                            ))}
                                        </List>
                                ...
                                <CardActions sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                    <Button disabled={shipment.status === ShipmentStatuses.DONE} size='medium' variant='contained' color='primary' onClick={() => setPendingShipmentToSolve(shipment)}>
            ...
            <Dialog open={Boolean(pendingShipmentToSolve)} onClose={() => setPendingShipmentToSolve(null)} dir="rtl">
                ...
                <DialogActions>
                    <Button onClick={() => setPendingShipmentToSolve(null)} color="primary" variant='contained'>الغاء</Button>
                    <Button onClick={() => handleSolve(pendingShipmentToSolve._id)} color='error' autoFocus variant='contained'>اكمال</Button>
                </DialogActions>
            </Dialog>
        </>
    )
